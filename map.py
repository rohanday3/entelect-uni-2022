        ...
        for y in range(self.height):
            for x in range(self.width):
                self.tiles[y][x] = Tile(
                    TileType[map[y][x]],
                    (y, x),
                    Resource.getItem(self.resources, [y, x]),
                )
    ...
    def print(self, resources=False):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                print(f"{colour}{self.tiles[y][x].type}{Style.RESET_ALL}", end=" ")
            print()
    ...
    def printPath(self, path):
        for y in range(self.height):
            for x in range(self.width):
                if (y, x) in path:
                    print(
                        Fore.RED + str(self.tiles[y][x].type) + Style.RESET_ALL, end=" "
                    )
                elif self.tiles[y][x].item != None:
                    print(
                        Fore.GREEN + str(self.tiles[y][x].type) + Style.RESET_ALL,
                        end=" ",
                    )
                else:
